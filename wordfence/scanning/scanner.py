...
class FileLocator:
    ...
    def _is_exempt_path(self, path: bytes) -> bool:
        """Check if path is /www or /staging (Flywheel hosting dirs)."""
        return path in [b'/www', b'/staging']

    def _is_root_or_nobody_owned(self, path: bytes) -> bool:
        """Check if a path is owned by root or nobody."""
        try:
            link_owner_id = os.lstat(path).st_uid
            link_owner_name = pwd.getpwuid(link_owner_id).pw_name
            return link_owner_name in {'root', 'nobody'}
    ...
    def search_directory(self, path: bytes, parents: Optional[list] = None):
        try:
            ...
            log.error('Directory ' + os.fsdecode(path) + ' does not exist.')
            return
        ...
        for item in contents:
            item_path = item.path
            try:
                is_exempt = self._is_exempt_path(item.path)

                if item.is_symlink():
                    item_path = os.path.realpath(item.path)
                    if item_path in self.scanned_paths:
                        continue
                    # Skip symlinks owned by root or nobody unless exempt
                    if not is_exempt and self._is_root_or_nobody_owned(
                            item.path):
                        log.warning(
                                'Skipping ' + os.fsdecode(item.path)
                                + ' symlink owned by root or nobody'
                            )
                        continue
                    # Bypass symlink loop check for exempt dirs
                    if not is_exempt and self._is_loop(item.path, parents):
                        continue
                if item.is_dir():
                    ...
                    try:
                        ...
                        log.warning(
                                'Skipping ' + os.fsdecode(item_path)
